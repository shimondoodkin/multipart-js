...
exports.EVENTS = [ "onPartBegin",'begin', "onPartEnd",'end', "onData",'data', "onEnd", "onError" , 'pause','resume' ];
  ...
  while (parser.buffer) {
    switch (parser.state) {
      case S.NEW_PART:
        ...
        // node's http client leaves \r\n after headers, clients send
        // \r\n\r\n between ending part boundaries. be liberal in
        // accepting input.
        parser.buffer = parser.buffer.replace(/^[\r\n]*/, '');
        ...
        if (parser.buffer.substr(len, 2) === "--") {
          ...
          this.emit("end", parser.part);       // stream competible part end
          this.emit("onPartEnd", parser.part); // should it be removed the api is new anyways.
      ...
      case S.HEADER:
        ...
        // let the world know
        this.emit("onPartBegin", parser.part); // maybe call it begin ?
      ...
      case S.BODY:
        ...
        if (offset === -1) {
          ...
          if (emittable > 0) {
            this.emit("data"  , parser.buffer.substr(0, emittable));  // stream competible
            this.emit("onData", parser.buffer.substr(0, emittable)); // should it be removed ? anyways it is a new version
        ...
        if (offset > 0) {
          ...
          if (emittable)
          {
           this.emit("data", emittable);  // stream competible
           this.emit("onData", emittable); // should it be removed ? anyways it is a new version
          }
        ...
        this.emit("end", parser.part);// stream competible part end
        this.emit("onPartEnd", parser.part);// should it be removed ? anyways it is a new version
...
Parser.prototype.end = function () { // new version stream competible
  this.emit("onEnd");
  Parser.call(this);
}
Parser.prototype.close = function () { // should this function be depriciated? anyways it is a new version
  this.emit("onEnd");
  Parser.call(this);
}


//Parser.prototype.pause = function () {this.emit("pause");};
//Parser.prototype.resume = function () {this.emit("resume"); };
