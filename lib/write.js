...
Writer.prototype.partBegin = function (part, headers) {
  ...
  this.emit("onData", partString + CRLF);
...
Writer.prototype.partEnd = function () {
  ...
  ;
	...
	this.emit("onData", CRLF + "--" + writer.boundary + "--" + CRLF);
...
Writer.prototype.close = function () {
  var writer = this;
  if (!writer.depth && writer.state === S.PART_ENDED) return this.emit("onEnd");
...
function writeHttpHeaders(writer) {
  ...
  this.emit("onData", headerString + CRLF + CRLF);
}
